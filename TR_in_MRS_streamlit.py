...
def page_snr():
    ...
    selected_Vs_plot = st.multiselect('Select V datasets for plot', v_options, ['All data sets'], key='plot_v')
...
def page_concentration():
    ...
    v_options = list(datasets.keys())
    v_options.insert(0, 'All data sets')
    selected_Vs_conc = st.multiselect('Select V datasets for plot', v_options, ['All data sets'], key='plot_v')  # Set 'All data sets' as the default option
    ...
    selected_metabolite_plot = st.selectbox('Select a metabolite for plot', metabolites_conc, key='plot_metabolite', index=metabolites_conc.index('NAA') if 'NAA' in metabolites_conc else 0)
    ...
    axs[0].set_title(f'Similar acq. time: reported conc. comparison for {label_map.get(selected_metabolite_plot, selected_metabolite_plot)}', fontsize=global_fontsize+4)
    axs[0].grid(True)
...
def page_CRLB():
    ...
    #             # CRLB_value_1 = filtered_df_plot_1['%CRLB'].values[0]
    #             combined_CRLB_value_1 += CRLB_value_1
    #         if not filtered_df_plot_2.empty:
    #             CRLB_value_2 = filtered_df_plot_2['mM CRLB'].values[0]
    #             # CRLB_value_2 = filtered_df_plot_2['%CRLB'].values[0]
    ...
    v_options = list(datasets.keys())
    v_options.insert(0, 'All data sets')
    selected_Vs_CRLBplot = st.multiselect('Select V datasets for plot', v_options, ['All data sets'], key='plot_CRLB_v')
    ...
    axs[0, 0].set_xlabel('TR (seconds)', fontsize=global_fontsize)
    axs[0, 0].set_ylabel('CRLB', fontsize=global_fontsize)
    axs[0, 0].set_title(f'Similar acq. time: CRLB comparison for {label_map.get(selected_metabolite_plot, selected_metabolite_plot)}', fontsize=global_fontsize+4)
    axs[0, 0].grid(True)
    axs[0, 0].set_xticks([2, 5, 8])
    axs[0, 0].tick_params(axis='both', labelsize=global_fontsize)

    axs[0, 1].set_xlabel('TR (seconds)', fontsize=global_fontsize)
    axs[0, 1].set_ylabel('CRLB', fontsize=global_fontsize)
    axs[0, 1].set_title(f'Same number of acqs.: CRLB comparison for {label_map.get(selected_metabolite_plot, selected_metabolite_plot)}', fontsize=global_fontsize+4)
    axs[0, 1].grid(True)
    axs[0, 1].set_xticks([2, 5, 8])
    axs[0, 1].tick_params(axis='both', labelsize=global_fontsize)

    axs[1, 0].set_xlabel('TR (seconds)', fontsize=global_fontsize)
    axs[1, 0].set_ylabel('%CRLB', fontsize=global_fontsize)
    axs[1, 0].set_title(f'Similar acq. time: %CRLB comparison for {label_map.get(selected_metabolite_plot, selected_metabolite_plot)}', fontsize=global_fontsize+4)
    axs[1, 0].grid(True)
    axs[1, 0].set_xticks([2, 5, 8])
    axs[1, 0].tick_params(axis='both', labelsize=global_fontsize)

    axs[1, 1].set_xlabel('TR (seconds)', fontsize=global_fontsize)
    axs[1, 1].set_ylabel('%CRLB', fontsize=global_fontsize)
    axs[1, 1].set_title(f'Same number of acqs.: %CRLB comparison for {label_map.get(selected_metabolite_plot, selected_metabolite_plot)}', fontsize=global_fontsize+4)
    axs[1, 1].grid(True)
    axs[1, 1].set_xticks([2, 5, 8])
    axs[1, 1].tick_params(axis='both', labelsize=global_fontsize)
    ...
    def plot_metabolite_data_single_plot(metab):
        ...
        ax.set_title(f'{metab} - mM and mM CRLB Values')
        ax.set_ylabel('Values')
        ax.set_ylim(bottom=start_val)  # Start y-axis at -0.02
        ax.set_xticks([1, 2])
        ax.set_xticklabels(['mM', 'mM CRLB'])
